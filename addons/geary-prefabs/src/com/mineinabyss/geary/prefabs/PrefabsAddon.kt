...
val Prefabs = gearyAddon("prefabs") {
    import(singleModule(SerializableComponents))
    single { new(::PrefabLoader) }
    single { new(::PrefabsModule) }

    addCloseables(get<PrefabLoader>(), get<PrefabsModule>())
...
fun DIScope.prefabs(configure: PrefabsModule.() -> Unit) = load(Prefabs, configure)
