...
public sealed class ObjectList<E>(initialCapacity: Int) {
    @JvmField
    var content: Array<Any?> = if (initialCapacity == 0) {
        EmptyArray
    } else {
 ...
 * It is common to use [MutableComponentList] internally and use [asMutableList] or [asList]
 ...
 * @eee MutableLongList
 */
public class MutableComponentList<E>(
    initialCapacity: Int = 16
) : ObjectList<E>(initialCapacity) {
    private var list: ObjectListMutableList<E>? = null

    /**
     * Returns the total number of elements that can be held before the [MutableComponentList] must
     * grow.
     *
     * @see ensureCapacity
     */
    ...
    /**
     * Adds [element] to the [MutableComponentList] and returns `true`.
     */
    ...
    /**
     * Adds [element] to the [MutableComponentList] at the given [index], shifting over any
    ...
    /**
     * Adds all [elements] to the [MutableComponentList] at the given [index], shifting over any
     * elements at [index] and after, if any.
     * @return `true` if the [MutableComponentList] was changed or `false` if [elements] was empty
     * @throws IndexOutOfBoundsException if [index] isn't between 0 and [size], inclusive.
     */
    public fun addAll(
        @androidx.annotation.IntRange(from = 0) index: Int,
    ...
    /**
     * Adds all [elements] to the end of the [MutableComponentList] and returns `true` if the
     * [MutableComponentList] was changed or `false` if [elements] was empty.
     */
    ...
    /**
     * Adds all [elements] to the end of the [MutableComponentList].
     */
    ...
    /**
     * Removes all elements in the [MutableComponentList]. The storage isn't released.
     * @see trim
     */
    public fun clear() {
    ...
    /**
     * Ensures that there is enough space to store [capacity] elements in the [MutableComponentList].
     * @see trim
     */
    ...
    /**
     * [add] [element] to the [MutableComponentList].
     */
    ...
    /**
     * [remove] [element] from the [MutableComponentList]
     */
    ...
    /**
     * Removes [element] from the [MutableComponentList]. If [element] was in the [MutableComponentList]
    ...
    /**
     * Removes all [elements] from the [MutableComponentList] and returns `true` if anything was removed.
     */
    ...
    /**
     * Removes all [elements] from the [MutableComponentList].
     */
    ...
    /**
     * Keeps only [elements] in the [MutableComponentList] and removes all other values.
     * @return `true` if the [MutableComponentList] has changed.
     */
    ...
    /**
     * Returns a [MutableList] view into the [MutableComponentList]. All access to the collection
     ...
     * areas should use the [MutableComponentList] API rather than [MutableList] API, when possible.
     */
    ...
    /**
     * [MutableList] implementation for a [MutableComponentList], used in [asMutableList].
     */
    private class ObjectListMutableList<T>(
        private val objectList: MutableComponentList<T>
    ) : MutableList<T> {
...
private val EmptyObjectList: ObjectList<Any?> = MutableComponentList(0)
 ...
 */
public fun <E> objectListOf(element1: E): ObjectList<E> = mutableComponentListOf(element1)
...
public fun <E> objectListOf(element1: E, element2: E): ObjectList<E> =
    mutableComponentListOf(element1, element2)
...
public fun <E> objectListOf(element1: E, element2: E, element3: E): ObjectList<E> =
    mutableComponentListOf(element1, element2, element3)

/**
 * @return a new read-only [ObjectList] with [elements] in order.
 */
public fun <E> objectListOf(vararg elements: E): ObjectList<E> =
    MutableComponentList<E>(elements.size).apply { plusAssign(elements as Array<E>) }

/**
 * @return a new empty [MutableComponentList] with the default capacity.
 */
public inline fun <E> mutableComponentListOf(): MutableComponentList<E> = MutableComponentList()

/**
 * @return a new [MutableComponentList] with [element1] as the only element in the list.
 */
public fun <E> mutableComponentListOf(element1: E): MutableComponentList<E> {
    val list = MutableComponentList<E>(1)
    list += element1
    return list
}

/**
 * @return a new [MutableComponentList] with 2 elements, [element1] and [element2], in order.
 */
public fun <E> mutableComponentListOf(element1: E, element2: E): MutableComponentList<E> {
    val list = MutableComponentList<E>(2)
    list += element1
    list += element2
    return list
}
...
/**
 * @return a new [MutableComponentList] with 3 elements, [element1], [element2], and [element3],
 * in order.
 */
public fun <E> mutableComponentListOf(element1: E, element2: E, element3: E): MutableComponentList<E> {
    val list = MutableComponentList<E>(3)
    list += element1
    list += element2
    list += element3
    return list
...
/**
 * @return a new [MutableComponentList] with the given elements, in order.
 */
public inline fun <E> mutableComponentListOf(vararg elements: E): MutableComponentList<E> =
    MutableComponentList<E>(elements.size).apply { plusAssign(elements as Array<E>) }
