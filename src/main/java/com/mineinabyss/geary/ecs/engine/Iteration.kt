...
inline fun Engine.forEach(vararg components: ComponentClass, andNot: Array<out ComponentClass> = emptyArray(), run: GearyEntity.(List<GearyComponent>) -> Unit) {
...
inline fun <reified T : GearyComponent> Engine.forEach(vararg andNot: ComponentClass = emptyArray(), run: GearyEntity.(T) -> Unit) {
    val tClass = T::class
...
inline fun <reified T : GearyComponent, reified T2 : GearyComponent> Engine.forEach(vararg andNot: ComponentClass = emptyArray(), run: GearyEntity.(T, T2) -> Unit) {
    val tClass = T::class
